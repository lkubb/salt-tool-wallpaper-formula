...
We will see a quick setup to configure the ``tool_wallpapers`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_wallpapers`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_wallpapers`` formulas under ``/srv/salt/tool_wallpapers/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_wallpapers/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_wallpapers/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_wallpapers/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_wallpapers/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_wallpapers`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_wallpapers`` formulas under ``/srv/salt/tool_wallpapers/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_wallpapers/parameters/roles

We will define 2 roles:

- ``tool_wallpapers/server``
- ``tool_wallpapers/client``

We create a configuration for the role ``tool_wallpapers/server`` in ``/srv/salt/tool_wallpapers/parameters/roles/tool_wallpapers/server.yaml``:
...
We create another configuration for the role ``tool_wallpapers/client`` in ``/srv/salt/tool_wallpapers/parameters/roles/tool_wallpapers/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_wallpapers/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_wallpapers/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_wallpapers/parameters/dns:domain/  '/srv/salt/tool_wallpapers/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-wallpapers-as-json:
      test.nop:
        - name: {{ wallpapers | json }}
...
.. code-block:: sls
    ...
    wallpapers-config-file-file-managed:
      file.managed:
        - name: {{ wallpapers.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='wallpapers-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ wallpapers.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``wallpapers`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ wallpapers['winner'] }}
